...
export default function AttendanceLog({ role, currentUserId, onOpenMember }: AttendanceLogProps) {
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true)
      try {
        const response = await apiFetch<{ month: string; members: AttendanceMemberSummary[] }>(`/team/attendance?month=${selectedMonth}`)
  ...
  function nextMonth() {
    const d = new Date(`${selectedMonth}-01T00:00:00`)
    d.setMonth(d.getMonth() + 1)
    if (d <= new Date()) setSelectedMonth(d.toISOString().slice(0, 7))
  }
